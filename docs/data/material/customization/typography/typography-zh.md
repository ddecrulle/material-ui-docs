...
接下来，您需要做的是修改主题，来使用这一个新的字体。 如果想在全局定义 Raleway 作为一个字体，您可以使用 [`CssBaseline`](/material/react-css-baseline/) 组件（或者你也可以选择你想要的任意其他 CSS 方案)。
...
`theme.typography.*` [variant](#variants) 属性会直接映射到生成的 CSS。 您可以在当中使用 [媒体查询（media queries）](/material/customization/breakpoints/#api)：
...
若你想实现此设置的自动化，则可以使用 [` responsiveFontSizes()`](/material/customization/theming/#responsivefontsizes-theme-options-theme) 的帮助程序将 Typography 的字体大小在主题设置为响应性。

{{"demo": "ResponsiveFontSizesChart.js", "hideToolbar": true}}

您可以在下面的示例中看到这个操作。 请尝试调整浏览器的窗口大小，您可以注意到当切换到不同的 [breakpoints](/material/customization/breakpoints/) 设置的宽度，字体的大小也随之改变。
...
默认情况下，typography object 为带有 [13 种变体](/material/react-typography/#component) ：
...
您可以使用 [主题探索功能](/material/customization/default-theme/?expand-path=$.typography) ，或者在此页面上打开 dev 工具控制（`window.theme.typogry`）来查看 typography 的默认值。
