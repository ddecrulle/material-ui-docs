...
The easiest way to add style overrides for a one-off situation is to use the [`sx` prop](/system/basics/#the-sx-prop) available on all Material-UI components. 下面是一个示例： Here is an example:
...
您可以在 [样式库互操作性](/material/guides/interoperability/) 指南中找到不同样式库的示例。
...
Sometimes, you can't use a CSS pseudo-class, as the state doesn't exist in the web specification. 我们以菜单项（menu item）组件和 _selected_ 状态为例。 有时候，您不能使用**伪类**，因为 web 标准中不存在该状态。 你可以使用全局类名`.Mui-selected`自定义`MenuItem`组件的特殊状态。
...
Please take a look at the theme's [global overrides page](/material/customization/theme-components/) for more details.
...
You can reference the [Styles library interoperability guide](/material/guides/interoperability/) to find examples of this using different styles libraries or plain CSS.
...
If you are already using the [CssBaseline](/material/react-css-baseline/) component for setting baseline styles, you can also add these global styles as overrides for this component. Here is how you can achieve the same by using this approach. Here is how you can achieve the same by using this approach.
...
Check out [ClassName Generator](/material/guides/classname-generator/) section for more details.
