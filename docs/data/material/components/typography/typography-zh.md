...
Material-UI **不会**自动加载 _Roboto_ 字体。 你负责加载你的应用程序中使用的任何字体。 有这样几个简单的方法来加载 Roboto 字体。 若想查询更高级的配置，请参阅[主题定制部分](/material/customization/typography/)。
...
某些情况下，您可能无法使用 `Typography` 组件。 希望您可以利用 theme 里 [`typography `](/material/customization/default-theme/?expand-path=$.typography) 的一些值。
...
- 您也可以 [使用 theme](/material/customization/theme-components/#default-props) 来修改全局字体映射。
...
除了使用默认的排版变体外，你还可以添加自定义的排版，或者禁用任何你不需要的排版。 更多信息请参见 [添加 & 禁用变体](/material/customization/typography/#adding-amp-disabling-variants) 示例。
...
- **字体大小（字阶）**。 请使用 [相对单位（rem）](/material/customization/typography/#font-size) 来适应用户的设置。
