...
对话框是 [modal](/material/react-modal/) 窗体的一种类型，它通常在应用程序内容之前呈现，来提供一些关键信息，或者要求用户做出决策。 对话框出现的时候会禁用应用程序的所有功能，只有被确认、被取消或已采取其他必要的操作时，对话框会从屏幕中消失。
...
您可以使用[`useMediaQuery`](/material/react-use-media-query/#usemediaquery)来实现一个全屏显示的对话框。
...
参考[对话框性能部分](/material/react-modal/#performance)。

## 设计局限

参考[对话框性能部分](/material/react-modal/#limitations)。

## 无障碍设计

参考[模态框无障碍设计部分](/material/react-modal/#accessibility)。
