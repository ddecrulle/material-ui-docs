...
The media query string can be any valid CSS media query, e.g. [`'(prefers-color-scheme: dark)'`](/material/customization/palette/#user-preference).
...
You can use MUI's [breakpoint helpers](/material/customization/breakpoints/) as follows:
...
Note: You can change the default options using the [`default props`](/material/customization/theme-components/#default-props) feature of the theme with the `MuiUseMediaQuery` key.
