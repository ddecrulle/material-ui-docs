...
若您需要一个更优雅的功能，譬如 combobox，multiselect，autocomplete，async 或者 creatable support，请查看 [`Autocomplete` 组件](/material/react-autocomplete/)。 此组件旨在改进 “react-select” 和 “downshift” 这两个包。

## 属性

选择器组件是通过自定义 [InputBase](/material/api/input-base/) 的 `<input>` 元素来实现的。 It extends the [text field components](/material/react-text-field/) sub-components, either the [OutlinedInput](/material/api/outlined-input/), [Input](/material/api/input/), or [FilledInput](/material/api/filled-input/), depending on the variant selected. 它有着相同的样式和许多相同的属性。 详情请参阅相应组件的 API 文档。
...
`TextField` wrapper 组件是一个完整的表单控件，它包括了标签，输入和帮助文本。 您可以在 [在此章节中](/material/react-text-field/#select) 查看使用 select 模式的示例。
