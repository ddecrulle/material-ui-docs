...
请参阅[Styled Engine guide](/material/guides/styled-engine/)，了解更多关于如何配置`styled-components`作为样式引擎的信息。
...
如果你使用的是 TypeScript，那么将需要使用 [模块扩展（module augmentation）](/material/guides/typescript/#customization-of-theme) 来让主题接收上述值。
...
[`Box`](/material/react-box/) 是一个轻量级组件，它可以以工具集的方式通过包装其他组件来达到访问其 `sx` 属性的目的。 默认情况下将渲染一个 `<div>` 元素。
